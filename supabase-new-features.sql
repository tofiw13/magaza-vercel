...
-- 6. SİFARİŞ STATUSU (ORDER STATUS TRACKING)
alter table public.orders add column if not exists status text default 'completed' check (status in ('pending', 'processing', 'completed', 'cancelled'));
alter table public.orders add column if not exists user_id text references auth.users(id) on delete set null;
...
-- 7. İSTİFADƏÇİ EMAIL TƏRCİHLƏRİ (NOTIFICATIONS)
...
-- Ratings: Hamı oxuya bilər, öz reytinqini yaza bilər
...
-- Loyalty points: Yalnız öz xalları
...
begin
  ...
  set rating_avg = (select coalesce(avg(rating), 0) from public.ratings where product_id = new.product_id),
      rating_count = (select count(*) from public.ratings where product_id = new.product_id)
...
-- Loyalty points avtomatik yarat
...
-- Nümunə flash sale (sabah üçün)
insert into public.flash_sales (product_id, discount_percent, original_price, sale_price, starts_at, ends_at)
select 'cv-template', 30, price, round(price * 0.7), now(), now() + interval '7 days'
from public.products where id = 'cv-template'
